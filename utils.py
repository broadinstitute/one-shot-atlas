...
def save_latent_space(data_frame, siamese_net, atlas='p56'):
    ...
    get_latent_space = K.function([siamese_net.layers[0].input], [siamese_net.get_layer("latent_space").get_output_at(1)])
...
def plot_predictions(siamese_net, data_frame, test_atlas='p56', reference_atlas='ccfv3', index=None, plot=True):
    ...
    #p_fit = np.abs(delta_x1 + delta_x2)/2
    #p_fit = float(int(p_fit * 131) * 10)
    ...
    if plot:
        ...
        #if test_atlas is not 'brainmaps':
            #axs[0].plot(x, ground_truth * 1310, label='ground truth')
        ...
        if not np.isnan(list(test.coordinate)[0]):
            axs[1].set_title('sample image ') #  + '('+str(list(test.coordinate)[0]*10)+' um)'
        else:
...
def plot_umap(new_dataframe, siamese_net):
    ...
    p1, _, _, _ = plot_predictions(siamese_net, new_dataframe, test_atlas='brainmaps', reference_atlas='ccfv3', index=ix, plot=False)
    p2, _, _, _ = plot_predictions(siamese_net, new_dataframe, test_atlas='brainmaps', reference_atlas='ccfv2', index=ix, plot=False)
    p3, _, _, _= plot_predictions(siamese_net, new_dataframe, test_atlas='brainmaps', reference_atlas='p56', index=ix, plot=False)
